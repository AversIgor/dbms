...
Поднимаются БД (`db`, порт из `POSTGRES_PORT`) и `migrate` (порт из `MIGRATE_PORT`). Страница схемы: http://127.0.0.1:8080/
...
Статус: http://127.0.0.1:8081/status  
Старт инкремента: http://127.0.0.1:8081/sync?start=1  
Аудит: http://127.0.0.1:8081/sync?audit=1  
Аудит с даты: http://127.0.0.1:8081/sync?audit=1&day=2026-08-30  
Стоп прогона: http://127.0.0.1:8081/sync?stop=1
...
На VPS поднимаются те же сервисы: PostgreSQL 16 + PostGIS (`db`) и `migrate` (страница схемы и миграции). Других приложений (`api`, `web`) в репозитории пока нет.
