...
Не отдельный HTTP-процесс: DDL — `migrate`, загрузка — fgislk, выдача внешним ИС — `api`. Карта, формы, XML, PDF и СМЭВ в контур не входят. Отдельный ГИС-сервер в контур не входит, пока не понадобится. В `admin` — карточка «Пространственные данные»: просмотр и удаление строк через HTTP `migrate` (`/rows`), без своего процесса.
...
- Схема таблиц меняется только через `migrate`; наполнение — fgislk; HTTP-выборки внешним ИС — `api`.

## Интерфейсы

- **migrate:** DDL таблиц Spatial data, расширение PostGIS.
- **fgislk:** запись семантики и контура из ИС ФГИС ЛК (СПД) в таблицы.
- **api:** read-only HTTP для внешних ИС; ключ связи — `fgis_id`; геометрию в первом методе не отдаёт.
- **admin:** нет HTTP этого раздела; карточка «Пространственные данные», строки — прокси к `migrate` `/rows`.
- Чтение: SQL к общей БД по **учётному номеру ФГИС ЛК** (`fgis_id`); субъект — фильтр; внешние ИС — HTTP раздела `api`.
