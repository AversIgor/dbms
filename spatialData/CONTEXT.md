...
Не отдельный HTTP-процесс: DDL — `migrate`, загрузка — fgislk, выдача внешним ИС — `api`. Карта, формы, XML, PDF и СМЭВ в контур не входят. Отдельный ГИС-сервер в контур не входит, пока не понадобится. В `admin` — карточка-заглушка (нет HTTP-процесса).
