...
- Параллелизм: `FGIS_MAX_WORKERS` (по умолчанию 5) — субъектов сразу, одна asyncio-задача на субъект. `FGIS_BATCH_WORKERS` (по умолчанию 3) — параллельных пачек карточек внутри субъекта. Живых HTTP к СПД на процесс = произведение. Пачка — один curl (`next`, reuse), не subprocess на id. Flush семантики каждые 100 карточек (`FLUSH_BATCH`).
...
- Настройки allowlist (`FGIS_MAX_WORKERS`, `FGIS_BATCH_WORKERS`, `FGIS_TLS`, `FGIS_HOST`) можно менять через `PUT /settings`: overlay-файл (`FGISLK_SETTINGS_FILE`, иначе `fgislk-settings.json` в cwd) поверх `.env`. Пул SQLAlchemy и кап HTTP к СПД с размера на старте не пересобираются. Рестарт без файла возвращает значения `.env`. Секреты и `POSTGRES_*` через API нельзя.
...
- **HTTP:** `/health`, `/ready`, `/status` (`day` = дата запроса `period_end`, `subject`, `data_kind`; `running` = живые воркеры; при совпадении `period_end` — `in_progress` / `result=running` / период / `updated_count`; прошлый журнал в `last`). `GET /panel` — манифест для admin (колонки, фильтры, пагинация по дате запроса). `GET /history` — журнал `fgis_import_history` (HTTP, не витрина). `GET`/`PUT /settings` — allowlist. Ручные прогоны (порт `FGISLK_PORT`, по умолчанию 8081):
  ...
  - один или несколько субъектов: `GET http://127.0.0.1:8081/sync?start=1&subject=07`, `GET http://127.0.0.1:8081/sync?start=1&subject=07,16,21,22,24`, `GET http://127.0.0.1:8081/sync?audit=1&subject=07`, `GET http://127.0.0.1:8081/sync?audit=1&day=2026-08-30&subject=07` (`stop` с `subject=` нельзя).
