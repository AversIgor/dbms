...
- Параллелизм: `FGIS_MAX_WORKERS` (по умолчанию 10) — живых HTTP к СПД на весь процесс и столько же субъектов сразу. Один субъект — одна asyncio-задача, пачка карточек — один curl (`next`, reuse соединения), не subprocess на id. Flush семантики каждые 100 карточек (`FLUSH_BATCH`).
...
- Настройки allowlist (`FGIS_MAX_WORKERS`, `FGIS_TLS`, `FGIS_HOST`) можно менять через `PUT /settings`: overlay-файл (`FGISLK_SETTINGS_FILE`, иначе `fgislk-settings.json` в cwd) поверх `.env`. Пул SQLAlchemy и кап HTTP к СПД с размера на старте не пересобираются. Рестарт без файла возвращает значения `.env`. Секреты и `POSTGRES_*` через API нельзя.
